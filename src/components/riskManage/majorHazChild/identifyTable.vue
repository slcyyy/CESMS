<template>
  ...
  <!--  -->
      <div class="horibox" style="width:100%;margin:10px 0 20px 0">
      <div class="horibox" :style="{visibility:(showTable?'visible':'hidden')}" >
        <el-input
          placeholder="请输入存储单元全称或关键字"
      ...
      <!-- 切换卡 -->
     <el-button-group>
         <el-tooltip content="编辑表格数据" placement="top">
        <el-button  icon="el-icon-edit-outline" :style="{'background':buttonColor1}" @click="switchData('1')"></el-button>
         </el-tooltip>
          <el-tooltip content="查看统计图数据" placement="top">
        <el-button  icon="el-icon-s-marketing" :style="{'background':buttonColor2}" @click="switchData('2')"></el-button>
          </el-tooltip>
      </el-button-group>
     </div>

   <!-- 统计图数据 -->
   <IdentifyChart v-if="showChart"></IdentifyChart>
   <!-- 表格数据 -->
    <el-table
      :data="tableData"
    ...
    >
      ...
      <el-table-column prop="chemi_device" label="设备" min-width="50%"></el-table-column>
      ...
      <el-table-column  prop="chemi_S" label="S" min-width="30%">
         <!-- <template v-slot="scope">{{computeS(scope.$index)}}</template> -->
      </el-table-column>
...
export default {
      ...
      return function(i){
         for(let j=0;j<this.unitOptions.length;j++){
          if(this.unitOptions[j].unit_id==this.tableData[i].chemi_unitId){
            if(this.unitOptions[j].unit_isRisk >0)
              return "是"
            else
              return "否"
          }
  ...
  methods: {
    ...
    async getTableData() {
      ...
      this.computeS(res.identifyResult)
      this.tableData = res.identifyResult
      if(this.change = true){

      }
      console.log('表格的数据是:',res.identifyResult)
      console.log('所有的单元格是:',this.unitOptions)
    },
    ...
    computeS(table){
      for(let i=0;i<this.unitOptions.length;i++){
        //当单元没有判断过是否为重大危险源时进行计算
        if(this.unitOptions[i].unit_isRisk==undefined || this.unitOptions[i].unit_isRisk==null){
          let index = 0
          this.change=true
          //遍历找到该存储单元的第一个存储数据
          for(let j=0;j<table.length;j++){
            if(table[j].chemi_unitId==this.unitOptions[i].unit_id){
               index = j
               break;
            }
          } 
          //危险源指标S
          let S = 0
          //unitlen为该单元内所有化学品的种类数目
          let unitlen = table[index].unitLength
          if(unitlen === 1){
            S='\\'   
            if(table[index].chemi_allowance>= table[index].chemi_criticalMass)
              this.unitOptions[i].unit_isRisk =1 //为1表示是重大危险源
            else this.unitOptions[i].unit_isRisk =0 //为0表示非重大危险源
          }
          else{
            for(let j = 0;j<unitlen;j++){
              S += (Number)(table[index+j].chemi_allowance/table[index+j].chemi_criticalMass)
            }
            S = S.toFixed(3)  
            if(S >=1)    this.unitOptions[i].unit_isRisk = 1
            else this.unitOptions[i].unit_isRisk =0
          }
          table[index].chemi_S = S
        }
      }
    },
    //合并行要先判断rowIndex，合并列要先判断columnIndex！不能用data()中数据进行自增
    arraySpanMethod({ row, column, rowIndex, columnIndex }) {
      if (columnIndex === 1 || columnIndex === 8 || columnIndex === 6 || columnIndex === 0) {
        if (row.unitLength) {
    ...
    async addUnitOptions(){
      let {data:res} = await this.$http.post('/hazard/editUnitRisk',{newUnit:this.unitOptions})
    ...
    showEditForm(index) {
      ...
      console.log('当前的editForm是：',this.oldEditForm)
    ...
    async editData() {
      ...
      if (JSON.stringify(this.editForm) == JSON.stringify(this.oldEditForm)) {
        ...
        let { data: res } = await this.$http.post('/hazard/editChemical', {
